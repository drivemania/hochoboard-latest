...
<div class="max-w-4xl mx-auto bg-white p-6 rounded-lg shadow-sm border border-neutral-200">
    <div class="border-b pb-4 mb-6">
        <h2 class="text-2xl font-bold text-neutral-800">{{ $board->title }} - 글쓰기</h2>
    </div>
    ...
    <form action="" method="POST" onsubmit="return confirm('등록하시겠습니까?');">
        ...
        <div class="flex items-center space-x-6 mb-4">
            ...
            <label class="flex items-center cursor-pointer">
                <input type="checkbox" name="is_notice" value="1" class="w-4 h-4 text-amber-600 rounded">
        ...
        <div class="mb-4">
            <input type="text" name="subject" class="w-full text-lg border-b-2 border-neutral-200 py-2 focus:outline-none focus:border-amber-600 transition placeholder-neutral-400" placeholder="제목을 입력하세요" required>
        </div>
                        ...
                        <div class="flex flex-wrap gap-3 mt-2">
                            @foreach($options as $opt)
                                <label class="flex items-center space-x-1 cursor-pointer">
                                    <input type="checkbox" name="custom[{{ $field['name'] }}][]" value="{{ trim($opt) }}" class="w-4 h-4 text-amber-600 rounded">
        ...
        <div class="mb-6">
            <textarea name="content" id="editor" class="w-full h-80 border rounded-lg p-4 focus:outline-none focus:ring-2 focus:ring-amber-200 resize-none" placeholder="내용을 입력하세요"></textarea>
        </div>

        <div class="flex justify-end space-x-2">
            <a href="{{ $currentUrl }}" class="px-5 py-2 rounded border border-neutral-300 text-neutral-600 hover:bg-neutral-50 transition">취소</a>
            <button type="submit" class="px-5 py-2 rounded bg-amber-600 text-white font-bold hover:bg-amber-700 transition shadow-md">등록하기</button>
        </div>
    </form>
</div>
