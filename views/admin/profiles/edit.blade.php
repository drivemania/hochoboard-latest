...
<div class="max-w-4xl mx-auto">
    <div class="mb-4">
        <a href="{{ $base_path }}/admin/profiles" class="text-neutral-500 hover:text-amber-500 text-sm font-bold">← 그룹 목록으로 돌아가기</a>
    </div>
    ...
    <form action="{{ $base_path }}/admin/profiles/update" method="POST">
        ...
        <div class="bg-white p-6 rounded-lg shadow-sm mb-6 border border-neutral-200">
            ...
            <div class="mb-6 bg-neutral-50 p-4 rounded border">
                <label class="font-bold text-neutral-700 block mb-2">프로필 입력 방식</label>
                <div class="flex space-x-6">
                    <label class="flex items-center cursor-pointer">
                        <input type="radio" name="use_fixed_char_fields" value="0" class="w-4 h-4 text-amber-500" 
                            {{ !$group->use_fixed_char_fields ? 'checked' : '' }}>
                        <span class="ml-2">자유 입력 (회원이 직접 항목 추가)</span>
                    </label>
                    <label class="flex items-center cursor-pointer">
                        <input type="radio" name="use_fixed_char_fields" value="1" class="w-4 h-4 text-amber-500"
                            {{ $group->use_fixed_char_fields ? 'checked' : '' }}>
            ...
            <div x-data="{ 
                ...
                <div class="space-y-3">
                    <template x-for="(field, index) in fields" :key="index">
                        <div class="flex flex-wrap items-start gap-2 p-3 bg-neutral-50 rounded border transition-all duration-200">
                            
                            <div class="flex-1 min-w-[150px]">
                                <label class="block text-xs text-neutral-500 mb-1">항목명</label>
                            ...
                            <div class="w-32">
                                <label class="block text-xs text-neutral-500 mb-1">타입</label>
                            ...
                            <div class="w-full mt-2 border-t pt-2" x-show="field.type === 'select'" x-transition>
                                <label class="block text-xs font-bold text-amber-500 mb-1">
                                    💡 선택지 입력 (쉼표 <code class="bg-neutral-200 px-1 rounded">,</code>로 구분)
                                </label>
                                <input type="text" 
                                       :name="`char_fields[${index}][options]`" 
                                       x-model="field.options" 
                                       class="w-full border border-amber-300 rounded px-2 py-1 text-sm bg-amber-50 placeholder-amber-300" 
                ...
                <div class="mt-4">
                    <button type="button" @click="addField()" class="w-full py-3 border-2 border-dashed border-neutral-300 rounded text-neutral-500 hover:text-amber-500 hover:border-amber-400 font-bold transition">
                        + 양식 항목 추가하기
                    </button>
                </div>
            </div>
        ...
        <div class="text-right">
            <button type="submit" class="bg-amber-500 text-white px-6 py-2 rounded font-bold hover:bg-amber-700 shadow-lg">
                설정 저장하기
            </button>
        </div>
    </form>
