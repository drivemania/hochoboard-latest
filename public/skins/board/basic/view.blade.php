...
<div class="max-w-4xl mx-auto bg-white p-6 rounded-lg shadow-sm border border-neutral-200">
    ...
    <div class="bg-neutral-50 p-4 rounded-lg">
        ...
        <ul class="space-y-4 mb-6">
            ...
            <li x-data="{ editMode: false }" class="border-b border-neutral-200 pb-2 last:border-0">
                    ...
                    <div class="flex items-center space-x-2">
                        ...
                        @if( (isset($_SESSION['user_id']) && $_SESSION['user_id'] == $cmt->user_id) || ($_SESSION['level'] ?? 0) >= 10 )
                            ...
                            <button type="button" 
                                    x-show="!editMode" 
                                    @click="editMode = true" 
                                    class="text-xs text-neutral-400 hover:text-amber-600">
                                수정
                            </button>
        ...
        <form action="{{ $currentUrl }}/comment" method="POST" class="flex items-start space-x-2">
            ...
            <button class="bg-amber-600 text-white px-4 py-2 rounded h-20 font-bold hover:bg-amber-700 text-sm">등록</button>
        </form>
        @else
