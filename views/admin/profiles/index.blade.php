...
<div class="bg-white rounded-lg shadow overflow-hidden">
    <div class="p-4 border-b bg-neutral-50">
        <p class="text-sm text-neutral-600">프로필 양식을 설정할 <b>커뮤니티 그룹</b>을 선택하세요.</p>
    </div>
    ...
    <table class="w-full text-left border-collapse">
        ...
        <tbody class="divide-y divide-neutral-200 text-sm">
            ...
            <tr class="hover:bg-neutral-50 transition">
                <td class="px-6 py-4 font-bold text-neutral-800">{{ $group->name }}</td>
                <td class="px-6 py-4 text-amber-500">/au/{{ $group->slug }}</td>
                <td class="px-6 py-4">
                    ...
                    @else
                        <span class="bg-neutral-100 text-neutral-500 px-2 py-1 rounded-full text-xs">자유 입력 모드</span>
                    @endif
                </td>
                <td class="px-6 py-4 text-right">
                    <a href="{{ $base_path }}/admin/profiles/{{ $group->id }}" class="bg-amber-500 text-white px-4 py-2 rounded hover:bg-amber-700 text-sm font-bold">
                        설정
                    </a>
                </td>
            </tr>
